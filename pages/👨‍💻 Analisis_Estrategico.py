...
with st.sidebar:
    ...
    cats_disp = ["TODAS"] + sorted(df['categoria_producto'].unique().tolist())
...
if cat_sel != "TODAS":
    df_analisis = df_analisis[df_analisis['categoria_producto'] == cat_sel]
...
margen_promedio = (total_margen / total_ventas * 100) if total_ventas else 0
...
with tab2:
    ...
    with col_b:
        ...
        df_group['Margen_Pct'] = (df_group['Margen_Pesos'] / df_group['Ventas']) * 100
        df_group['Peso_Venta_Pct'] = (df_group['Ventas'] / df_group['Ventas'].sum()) * 100
...
with tab3:
    if anio_comparar == "Ninguno":
        ...
        if cat_sel != "TODAS": df_prev = df_prev[df_prev['categoria_producto'] == cat_sel]
...
with tab4:
    ...
    **Metodología:** Este módulo analiza la distribución de ventas según el centro de operación (Asumiendo zonas de vendedores).
    ...
    # Definimos centros de operación basados en los grupos de vendedores (Proxy de Ubicación)
    # Si existiera una columna 'Población' real, se usaría aquí.
    
    df_geo = df_analisis.copy()
    
    # Asignar Zona basada en Grupo de Vendedor si es posible
    def asignar_zona(row):
        vendedor = normalizar_texto(row['nomvendedor'])
        if "PEREIRA" in vendedor: return "Eje Cafetero - Risaralda"
        if "ARMENIA" in vendedor: return "Eje Cafetero - Quindío"
        if "MANIZALES" in vendedor: return "Eje Cafetero - Caldas"
        if "OPALO" in vendedor: return "Digital / Remoto"
        return "Otras Zonas / Viajeros"
    
    # Buscamos si podemos mapear el grupo desde DATA_CONFIG en session_state
    grupos_config = st.session_state.DATA_CONFIG['grupos_vendedores']
    ...
    def map_zona_avanzado(nom_vendedor):
        ...
        for grupo, vendedores in grupos_config.items():
            if nom_norm in [normalizar_texto(v) for v in vendedores]:
                return grupo # El nombre del grupo suele ser la ciudad (MOSTRADOR PEREIRA)
        return "ZONA GENERAL"
    ...
    df_zona['Ticket_Promedio_Zona'] = df_zona['Ventas'] / df_zona['Transacciones']
    df_zona['Margen_Pct_Zona'] = (df_zona['Margen'] / df_zona['Ventas']) * 100
...
# ==============================================================================
# FUNCIONES AUXILIARES (Requeridas para el script independiente)
# ==============================================================================
