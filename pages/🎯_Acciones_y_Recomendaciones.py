# ==============================================================================
# SCRIPT COMPLETO PARA: pages/1_Acciones_y_Recomendaciones.py
# VERSIÓN ORGANIZADA
# DESCRIPCIÓN: Script para la página de análisis estratégico. Se ha organizado
#              internamente para mejorar la claridad, añadiendo docstrings y
#              agrupando la lógica en secciones bien definidas.
# ==============================================================================
...
# - Se definen funciones de ayuda inicial.
# - Se verifica la autenticación del usuario y la existencia de datos.
# ==============================================================================
...
def normalizar_texto(texto):
    ...
    para facilitar las comparaciones de texto.
    """
...
def mostrar_acceso_restringido():
    """
    Muestra un mensaje y detiene la ejecución si el usuario no ha iniciado sesión.
    """
...
if df_ventas_historico is None or df_ventas_historico.empty or not APP_CONFIG or not DATA_CONFIG:
    st.error("No se pudieron cargar los datos maestros. Vuelva a la página principal y recargue los datos.")
    st.stop()


# ==============================================================================
# SECCIÓN 2: LÓGICA DE ANÁLISIS Y PROCESAMIENTO (El "Cerebro")
# - Contiene todas las funciones que realizan los cálculos y análisis de datos.
# - Incluye análisis de rentabilidad, segmentación RFM y matriz de productos.
# ==============================================================================

@st.cache_data
def preparar_datos_y_margen(df):
    """
    Separa el dataframe de ventas en productos y descuentos, y calcula el margen bruto.
    """
    df_copy = df.copy()
    df_copy['nombre_articulo_norm'] = df_copy['nombre_articulo'].str.upper()
    filtro_descuento = (df_copy['nombre_articulo_norm'].str.contains('DESCUENTO', na=False)) & \
                       (df_copy['nombre_articulo_norm'].str.contains('COMERCIAL', na=False))
...
def analizar_rentabilidad(df_productos, df_descuentos):
    ...
    Retorna un diccionario con todos los resultados del análisis.
    """
...
def analizar_segmentacion_rfm(df_productos, fecha_fin_analisis_dt):
    """
    Realiza la segmentación de clientes usando el modelo RFM (Recencia, Frecuencia, Monetario).
    """
...
def analizar_matriz_productos(df_productos):
    """
    Clasifica los productos en una matriz de tipo BCG (Estrella, Vaca, Interrogante, Perro).
    """
    if df_productos.empty: return pd.DataFrame()
...
def generar_excel_descargable(datos_para_exportar):
    ...
    para que el usuario pueda descargarlo.
    """
    output = io.BytesIO()
...
# ==============================================================================
# SECCIÓN 3: INTERFAZ DE USUARIO (UI) Y EJECUCIÓN PRINCIPAL
# - Función principal que construye la página de Streamlit.
# - Organiza los filtros, llama a las funciones de análisis y muestra los resultados.
# ==============================================================================

def render_pagina_acciones():
    """
    Renderiza toda la página de Acciones y Recomendaciones, desde los filtros
    hasta los gráficos y tablas finales.
    """
    ...
    # --- Bloque 1: Filtros y Selección de Datos ---
    st.subheader("1. Seleccione el Foco del Análisis")

    # Preparar listas de vendedores y grupos para el selector
    ...
    # Ajustar opciones según el tipo de usuario (Gerente vs Vendedor)
    ...
    if len(opciones_slider) > 1:
        ...
        st.info(f"Mostrando análisis para el único periodo disponible: {mes_inicio_str}")
    else:
    ...
    # --- Bloque 2: Ejecución de Análisis y Botón de Descarga ---
    st.markdown("---")
    ...
    st.download_button(
        ...
        data=generar_excel_descargable({
            ...
            "Evolucion_Rentabilidad": analisis_rentabilidad['df_evolucion'], 
            "Top_Clientes_con_Dcto": analisis_rentabilidad['top_clientes_descuento']
        }),
        file_name=f"Plan_Accion_{seleccion.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.xlsx",
    ...
    col1.metric("Margen Bruto (Productos)", f"${analisis_rentabilidad['margen_bruto_productos']:,.0f}")
    ...
    col4.metric("% Descuento sobre Venta Bruta", f"{analisis_rentabilidad['porcentaje_descuento']:.1f}%", help="(Total Descuentos / Venta Bruta de Productos) * 100")

    df_evo = analisis_rentabilidad['df_evolucion']
    if not df_evo.empty:
        fig_evo = px.line(df_evo, x='mes_anio', y=['margen_bruto', 'margen_operativo'], title="Evolución Mensual: Margen Bruto vs. Margen Operativo", labels={"value": "Monto ($)", "mes_anio": "Mes"}, markers=True)
        fig_evo.update_layout(legend_title_text='Leyenda')
        st.plotly_chart(fig_evo, use_container_width=True)
        st.info("💡 La brecha entre las dos líneas representa el impacto de los descuentos comerciales otorgados cada mes.")
    
    st.subheader("Clientes con Mayor Monto de Descuento Otorgado")
    st.dataframe(analisis_rentabilidad['top_clientes_descuento'], use_container_width=True, hide_index=True, column_config={"valor_venta": st.column_config.NumberColumn("Total Descuento", format="$ %d"), "nombre_cliente": "Nombre del Cliente"})

    # 3.2 Segmentación de Clientes (RFM)
    ...
    with st.container(border=True):
        ...
        if not df_matriz_productos.empty:
            fig_matriz = px.scatter(
                df_matriz_productos, 
                x="Volumen", y="Rentabilidad", 
                color="Segmento", size='Volumen', 
                hover_name="nombre_articulo", log_x=True, 
                color_discrete_map={'⭐ Estrella': 'gold', '🐄 Vaca Lechera': 'dodgerblue', '❓ Interrogante': 'limegreen', '🐕 Perro': 'tomato'},
                title="Matriz de Rendimiento de Productos (Volumen vs. Rentabilidad)"
            )
            st.plotly_chart(fig_matriz, use_container_width=True)
            
            st.subheader("Explorar Datos de Productos por Segmento")
            ...
            if not df_filtrada.empty:
                ...
                st.dataframe(df_filtrada, use_container_width=True, hide_index=True, height=350, column_config={
                    "Volumen": st.column_config.NumberColumn("Venta Total", format="$ %d"), 
                    "Rentabilidad": st.column_config.ProgressColumn(
                        "Rentabilidad (%)",
                        format="%.1f%%", 
                        min_value=float(min_rentabilidad - abs(min_rentabilidad * 0.1) if min_rentabilidad != 0 else -10), 
                        max_value=float(max_rentabilidad + abs(max_rentabilidad * 0.1) if max_rentabilidad != 0 else 10)
                    )
                })
        else:
            st.warning("No hay suficientes datos de productos para generar la matriz en este periodo.")

# --- Punto de entrada para la ejecución de la página ---
