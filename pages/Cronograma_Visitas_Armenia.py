...
<style>
    ...
    /* Alertas Personalizadas (Tu diseño original) */
    .alerta-compra {
    ...
    .ia-highlight { color: #4ade80; font-weight: bold; } /* Verde neón */
    .ia-danger { color: #f87171; font-weight: bold; } /* Rojo neón */
    .ia-command { 
...
with st.sidebar:
    ...
    st.divider()
    st.write("Versión del Sistema: 3.2 Ultimate")
...
# SOLUCIÓN DEL ERROR: Ahora recibimos 4 columnas, y la función devuelve 4 valores.
datos_calculados = df_proyectos.apply(
    lambda x: gestor.calcular_potencial_compra(x['m2_aprox'], x['Etapa'], x['Tipo']), 
    axis=1, 
    result_type='expand'
)
# Asignamos las 4 columnas correctamente
...
# --- ESTRUCTURA DE PESTAÑAS (AQUÍ ESTÁ LA MAGIA NUEVA) ---
...
with pestana_dashboard:
    ...
    # --- MENSAJE FINAL MOTIVACIONAL (ORIGINAL) ---
    st.markdown("""
...
with pestana_ia:
    ...
    # Análisis lógico para construir el texto de la IA
    ...
    # TEXTO GENERADO (SIMULACIÓN DE IA AVANZADA)
    mensaje_ia = f"""
