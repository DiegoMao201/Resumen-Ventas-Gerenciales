...
<style>
    div[data-testid="stButton"] > button[kind="primary"] {
        height: 3em; font-size: 1.2em; font-weight: bold; border: 2px solid #FF4B4B; background-color: #FF4B4B; color: white;
    }
    ...
    try:
        with dropbox.Dropbox(app_key=st.secrets.dropbox.app_key, app_secret=st.secrets.dropbox.app_secret, oauth2_refresh_token=st.secrets.dropbox.refresh_token) as dbx:
            ...
            if df.shape[1] != len(nombres_columnas):
                st.warning(f"⚠️ **Alerta de Carga de Datos en '{ruta_archivo}'**: Se detectaron filas con un número de columnas incorrecto. Algunas filas podrían haber sido descartadas. Se esperaban {len(nombres_columnas)} columnas pero se encontraron {df.shape[1]}.")
            ...
            for col in numeric_cols_to_clean:
                if col in df.columns:
                    ...
                    is_in_parens = s.str.startswith('(') & s.str.endswith(')')
                    ...
                    numeric_s.loc[is_in_parens] = -numeric_s.loc[is_in_parens].abs()
                    df[col] = numeric_s.fillna(0)
            ...
            if rows_before_dropna > rows_after_dropna:
                st.warning(f"⚠️ **Alerta de Limpieza en '{ruta_archivo}'**: Se eliminaron {rows_before_dropna - rows_after_dropna} filas porque no tenían un valor válido en 'año' o 'mes'.")
...
# ==============================================================================
# FUNCIÓN DE PROCESAMIENTO PRINCIPAL (CORREGIDA Y SIMPLIFICADA)
# ==============================================================================
def procesar_datos_periodo(df_ventas_periodo, df_cobros_periodo):
    ...
    # Los albaranes pendientes son simplemente los que la consulta ya etiquetó como tal.
    df_albaranes_pendientes_reales = df_ventas_periodo[df_ventas_periodo['TipoDocumento'] == 'ALBARAN PENDIENTE'].copy()

    # --- 3. AGRUPAR DATOS PARA EL RESUMEN ---
    # Ahora todos los cálculos se basan en los DataFrames limpios y correctos de arriba.
    ...
    # El cálculo de la marquilla puede usar todos los datos del periodo
    ...
    df_ventas_historicas_kpi = st.session_state.df_ventas[st.session_state.df_ventas['TipoDocumento'].isin(tipos_documento_kpi)].copy()
    ...
    for grupo, lista_vendedores in DATA_CONFIG['grupos_vendedores'].items():
        ...
        if not df_grupo_actual.empty:
            anio_sel = df_ventas_periodo['anio'].iloc[0]
            mes_sel = df_ventas_periodo['mes'].iloc[0]
...
def render_analisis_detallado(df_vista, df_ventas_periodo):
    ...
    if enfoque_sel == "Visión General":
        df_ventas_enfocadas = df_ventas_periodo
        df_ranking = df_vista
    else:
        enfoque_sel_norm = normalizar_texto(enfoque_sel)
        nombre_grupo_orig = next((k for k, v in DATA_CONFIG['grupos_vendedores'].items() if enfoque_sel_norm == normalizar_texto(k)), enfoque_sel_norm)
        
        nombres_a_filtrar = [normalizar_texto(n) for n in DATA_CONFIG['grupos_vendedores'].get(nombre_grupo_orig, [enfoque_sel])]
    ...
    with tab1:
        ...
        with col1:
            ...
            if not df_ventas_enfocadas_kpi.empty and 'nombre_marca' in df_ventas_enfocadas_kpi.columns and 'valor_venta' in df_ventas_enfocadas_kpi.columns:
                ...
                fig = px.treemap(df_marcas, path=[px.Constant("Todas las Marcas"), 'nombre_marca'], values='valor_venta', title="Composición de Venta por Marca")
                st.plotly_chart(fig, use_container_width=True)
        with col2:
            st.markdown("##### Ventas de Marquillas Clave")
            # Para este gráfico, usamos el df_ventas_periodo completo para incluir albaranes
            if not df_ventas_periodo.empty and 'nombre_articulo' in df_ventas_periodo.columns and 'valor_venta' in df_ventas_periodo.columns:
                ventas_marquillas = {p: df_ventas_periodo[df_ventas_periodo['nombre_articulo'].str.contains(p, case=False, na=False)]['valor_venta'].sum() for p in APP_CONFIG['marquillas_clave']}
...
def render_dashboard():
    ...
    if df_ventas_periodo.empty:
        ...
        if df_vista.empty:
            ...
            if st.button("🔄 ¡Actualizar Todos los Datos!", type="primary", use_container_width=True, help="Fuerza la recarga de los archivos desde Dropbox."):
                st.cache_data.clear()
                st.session_state.pop('df_ventas', None)
            ...
            if usuario_actual_norm != "GERENTE" and nombre_a_filtrar != usuario_actual_norm:
            ...
            st.dataframe(df_vista[cols_desglose], use_container_width=True, hide_index=True, column_config={
                "Estatus": st.column_config.TextColumn("🚦", width="small"), "nomvendedor": "Vendedor/Grupo", 
                "ventas_totales": st.column_config.NumberColumn("Ventas Reales", format="$ %d"), "presupuesto": st.column_config.NumberColumn("Meta Ventas", format="$ %d"),
                "cobros_totales": st.column_config.NumberColumn("Recaudo", format="$ %d"),"presupuestocartera": st.column_config.NumberColumn("Meta Recaudo", format="$ %d"),
                "albaranes_pendientes": st.column_config.NumberColumn("Valor Albaranes", format="$ %d"),"impactos": st.column_config.NumberColumn("Clientes Únicos", format="%d"),
            ...
            for vendedor in vendedores_vista_actual:
                ...
                nombre_grupo_orig = next((k for k,v in DATA_CONFIG['grupos_vendedores'].items() if vendedor_norm == normalizar_texto(k)), vendedor_norm)
                lista_vendedores = DATA_CONFIG['grupos_vendedores'].get(nombre_grupo_orig, [vendedor])
            ...
            if not df_albaranes_vista.empty:
                st.dataframe(df_albaranes_vista[['Serie', 'fecha_venta', 'nombre_cliente', 'valor_venta', 'nomvendedor']], 
                    use_container_width=True, hide_index=True, column_config={
                        "Serie": "Documento", "fecha_venta": "Fecha", "nombre_cliente": "Cliente",
                        "valor_venta": st.column_config.NumberColumn("Valor Pendiente", format="$ %d"), "nomvendedor": "Vendedor"
                    })
            else:
                st.success("¡Excelente! No hay albaranes pendientes de facturar para la selección actual en este periodo.", icon="✅")
...
def main():
    ...
    if not st.session_state.autenticado:
        ...
        def obtener_lista_usuarios(df):
            if df.empty or 'nomvendedor' not in df.columns:
                return ["GERENTE"] + sorted(list(DATA_CONFIG['grupos_vendedores'].keys()))
            ...
            vendedores_unicos_norm = set(df['nomvendedor'].dropna().unique())
            
            vendedores_solos_norm = sorted([v for v in vendedores_unicos_norm if v not in vendedores_en_grupos_norm and v not in {normalizar_texto(g) for g in grupos_orig}])
            
            # Para la lista de login, mostramos los nombres originales de los grupos
            return ["GERENTE"] + sorted(grupos_orig) + vendedores_solos_norm
        ...
        if st.sidebar.button("Salir", key="btn_logout"):
            for key in list(st.session_state.keys()):
                if key != 'df_ventas' and key != 'df_cobros':
