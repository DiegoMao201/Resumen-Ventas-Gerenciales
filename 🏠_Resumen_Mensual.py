...
def calcular_marquilla(df_periodo):
    ...
    # Crear una columna por cada palabra clave
    ...
    # Agrupar por cliente para ver qué marquillas únicas compró
    df_marquilla_cliente = df_periodo.groupby(['codigo_vendedor', 'nomvendedor', 'cliente_id']).agg({
        f'compro_{palabra.lower()}': 'any' for palabra in MARQUILLAS_CLAVE
    }).reset_index()
    
    # Sumar los puntos por cliente (de 0 a 4)
...
# ... (El resto de las funciones como generar_comentario_asesor y la de autenticación se mantienen igual, se omiten aquí por brevedad)
...
def render_dashboard():
    # ... (Se mantiene la primera parte de la función render_dashboard: filtros, procesamiento principal, KPIs, tabla resumen)
    st.sidebar.markdown("---")
    ...
    # Lógica para el filtro de enfoque
    if len(dff['nomvendedor'].unique()) > 1:
    ...
    elif enfoque_sel in GRUPOS_VENDEDORES:
    ...
    with col_graf1:
        st.subheader("🏆 Composición de Ventas por Marca")
        if not df_ventas_enfocadas.empty:
            df_marcas = df_ventas_enfocadas.groupby('nombre_marca')['valor_venta'].sum().reset_index()
            fig_treemap = px.treemap(df_marcas, path=['nombre_marca'], values='valor_venta', title="Distribución de Ingresos por Marca")
            fig_treemap.update_layout(margin=dict(t=50, l=25, r=25, b=25))
            st.plotly_chart(fig_treemap, use_container_width=True)
        else: st.info("No hay datos de ventas de productos para mostrar.")

    with col_graf2:
        st.subheader("🎨 Ventas de Marquillas Clave")
        if not df_ventas_enfocadas.empty:
            df_ventas_enfocadas['nombre_articulo'] = df_ventas_enfocadas['nombre_articulo'].astype(str)
            ventas_marquillas = {}
            for palabra in MARQUILLAS_CLAVE:
                total_venta = df_ventas_enfocadas[df_ventas_enfocadas['nombre_articulo'].str.contains(palabra, case=False)]['valor_venta'].sum()
                ventas_marquillas[palabra] = total_venta
            ...
            fig_marquillas = px.bar(df_ventas_marquillas, x='Marquilla', y='Ventas', text_auto='.2s', title="Total Vendido por Marquilla Clave")
            fig_marquillas.update_traces(textangle=0, textposition='outside')
            st.plotly_chart(fig_marquillas, use_container_width=True)
        else:
            st.info("No hay datos de ventas de marquillas para mostrar.")
