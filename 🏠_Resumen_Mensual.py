...
# (Omitidos por brevedad para no repetir, pero son los mismos que ya teníamos)
...
def render_dashboard():
    ...
    st.markdown(f"**Vista para:** `{st.session_state.usuario if len(dff) == 1 else 'Múltiples Seleccionados'}`")
    st.markdown("---")
    ...
    # ... (código de métricas y barras de progreso)
    ...
    df_ventas_enfocadas = pd.DataFrame()
    if enfoque_sel == "Visión General":
        nombres_vendedores_enfocados = dff['nomvendedor'].tolist()
    ...
    st.subheader("📊 Ranking de Cumplimiento de Meta")
    if not df_enfocado_graficos.empty and 'presupuesto' in df_enfocado_graficos.columns:
        ...
        df_ranking = df_ranking.sort_values('avance_ventas', ascending=False)
