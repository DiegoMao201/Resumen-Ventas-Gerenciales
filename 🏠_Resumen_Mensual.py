...
def calcular_marquilla(df_periodo):
    """Calcula el promedio de marquilla para cada vendedor."""
    if df_periodo.empty:
    ...
    df_marquilla_cliente = df_periodo.groupby(['codigo_vendedor', 'nomvendedor', 'cliente_id']).agg({
        f'compro_{palabra.lower()}': 'any' for palabra in palabras_clave
    }).reset_index()
...
def generar_comentario_asesor(avance_v, avance_c, marquilla_p):
    """Genera comentarios dinámicos basados en el rendimiento."""
    comentarios = []
    if avance_v >= 100:
        comentarios.append("📈 **Ventas:** ¡Felicitaciones! Has superado la meta de ventas. ¡Excelente trabajo!")
    elif avance_v >= 80:
        comentarios.append("📈 **Ventas:** ¡Estás muy cerca de la meta! Un último esfuerzo para lograrlo.")
    else:
        comentarios.append("📈 **Ventas:** Planifica tus visitas y aprovecha cada oportunidad.")

    if avance_c >= 100:
        comentarios.append("💰 **Cartera:** Objetivo de recaudo cumplido. Una gestión impecable.")
    else:
        comentarios.append("💰 **Cartera:** Recuerda hacer seguimiento a la cartera pendiente. Una buena gestión de cobro es clave.")

    if marquilla_p >= META_MARQUILLA:
        comentarios.append(f"🎨 **Marquilla:** Tu promedio de {marquilla_p:.2f} es excelente.")
    elif marquilla_p > 0:
        comentarios.append(f"🎨 **Marquilla:** Tu promedio es {marquilla_p:.2f}. Recuerda ofrecer todo el portafolio de marcas clave.")
    else:
        comentarios.append("🎨 **Marquilla:** Aún no registras ventas en las marcas clave. ¡Son una gran oportunidad de crecimiento!")
    return comentarios
...
def render_dashboard():
    ...
    anio_sel = st.sidebar.selectbox("Elija el Año", lista_anios, key="anio_selector")
    lista_meses_num = sorted(df_ventas[df_ventas['anio'] == anio_sel]['mes'].unique())
    mes_sel_num = st.sidebar.selectbox("Elija el Mes", options=lista_meses_num, format_func=lambda x: MAPEO_MESES.get(x), key="mes_selector")
    ...
    # --- FLUJO DE PROCESAMIENTO REESTRUCTURADO Y CORREGIDO ---
    # 1. Crear resumen base para individuos
    ...
    # 3. Asignar presupuestos a los individuos
    ...
    # 4. Llenar valores nulos DESPUÉS de todas las uniones
    df_resumen_completo.fillna(0, inplace=True)
    
    # 5. Crear los grupos a partir de la tabla individual completa
    registros_agrupados = []
...
def main():
    ...
    # El formulario de login siempre se muestra en la barra lateral
    ...
    # Decidir qué mostrar en la página principal
